...
def build_model(data, parameters):
        ...
        for reception in data['receptions']:
            for plant in data['plants']:
                for item in data['items']:
                    ...
                    Xsrpi[supplier, reception, plant, item] = model.addVar(lb=0, ub=None, name=variable_name, vtype="INTEGER")
    ...
    for s in data['suppliers']:
        for r in data['receptions']:
            for p in data['plants']:
                for i in data['items']:
                    model.addCons(Psrpi[s,r,p,i] <= Xsrpi[s,r,p,i] * parameters['Wp'][i])
    ...
    for r in data['receptions']:
        for p in data['plants']:
                model.addCons(pyscipopt.quicksum(Yrpic[r,p,i,c] for i in data['items']) <= parameters['RCAd'][r] )
    ...
    for r in data['receptions']:
        for p in data['plants']:
                model.addCons(pyscipopt.quicksum(Yrpic[r,p,i,c] for i in data['items']) <= parameters['RCMd'][r])

    # Peso limite de corridor automatico
    for r in data['receptions']:
        for p in data['plants']:
            model.addCons(pyscipopt.quicksum(Psrpi[s,r,p,i] * parameters['Fp'][i] for s in data['suppliers'] for i in data['items'])
                          <= parameters['RWAd'][r])

    # stock de supplier
    for s in data['suppliers']:
        model.addCons(pyscipopt.quicksum(Psrpi[s,r,p,i] for r in data['receptions'] for p in data['plants'] for i in data['items'])
                      <= parameters['Ss'][s])
    ...
    for r in data['receptions']:
        for p in data['plants']:
                model.addCons(pyscipopt.quicksum(Yrpic[r,p,i,c] for i in data['items']) <= parameters['Eda'][(r,p)]*MM)
